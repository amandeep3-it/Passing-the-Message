...
package Generic_Message_Structures is
   ...
   -- Keep Track of other Router's Distance and Neighbor to choose
   type Path is record
      Distance    : Natural      := 0;
      Router      : Router_Range := Router_Range'Invalid_Value;
   end record;
   -- 2D array to store the Dijkstra's table
   type Paths is array (Router_Range'Range) of Path;

   -- To Pass Message to Neighbors
   type Message_Delivery is record
      Sender      : Router_Range     := Router_Range'Invalid_Value;
